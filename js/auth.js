  ...
  class TamilAuthEngine {
          ...
          <div class="header-user-nav">
            ${
              isLoggedIn
                ...
                `
                : `
                  <button type="button" class="google-auth-btn-hdr" onclick="TamilAuth.loginWithGoogle()" title="Google Login">
                    ...
                    <span class="google-btn-text">Google Login</span>
                  </button>
      ...
      const modalHTML = `
        <div id="tamil-auth-modal-root" class="auth-modal-overlay">
          <div class="auth-modal-card">
            ...
            <div class="auth-divider">${isEn ? 'OR' : 'அல்லது'}</div>

            <button type="button" class="google-auth-btn" onclick="TamilAuth.loginWithGoogle()">
              <svg width="18" height="18" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/>
              ...
              Google ${isEn ? 'Sign in with Google' : 'மூலம் உள்நுழைக'}
            </button>
          </div>
        </div>
      `;
    ...
    loginWithGoogle() {
      if (typeof firebase === 'undefined' || !firebase.auth) {
        ...
        script.onload = () => this.executeGooglePopup();
        document.head.appendChild(script);
      } else {
        this.executeGooglePopup();
      }
    }
    ...
    executeGooglePopup() {
      ...
      firebase.auth().signInWithPopup(provider).then((result) => {
        ...
        if (!existing) {
          this.register(name, email, 'google_oauth_pass', ROLES.GUEST);
          existing = this.getUsersDB().find(u => u.email.toLowerCase() === email.toLowerCase());
        }

        this.login(email, existing ? existing.password : 'google_oauth_pass');
        this.showAlert(this.t('loginSuccess'), 'success');
        ...
        this.showAlert(err.message || 'Google Auth Error', 'error');
      });
    }
